    ...
    public class GridPager<T> : IGridPager<T>
    {
        ...
        {            
            get
            {
                ...
                CurrentPageValue = Int32.TryParse(Grid.Query[prefix + "Page"], out page) ? page : CurrentPageValue;
        ...
        public virtual Int32 RowsPerPage
        {
            get
            {
                int rows;
                String prefix = String.IsNullOrEmpty(Grid.Name) ? "" : Grid.Name + "-";
                RowsPerPageValue = Int32.TryParse(Grid.Query[prefix + "Rows"], out rows) ? rows : RowsPerPageValue;
        ...
        public virtual IQueryable<T> Process(IQueryable<T> items)
        {
            ...
            return items.Skip((CurrentPage - 1) * RowsPerPage).Take(RowsPerPage).AsQueryable();
        }
    }
}
